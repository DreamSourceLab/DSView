...
static int check_pattern_file_is_valid(const char * path)
{
    unzFile temp = unzOpen64(path);
    if(temp != NULL)
        return SR_OK;
    else   
        return SR_ERR;
 ...
 (void)cg;
    ...
    case SR_CONF_PATTERN_MODE:
        ...
        if(get_pattern_mode_index_by_string(sdi->mode , stropt) != PATTERN_INVALID)
        {
            ...
            get_dsl_path_by_pattern_mode(sdi->mode,cur_sample_generator);
        }
//        //字符串有效
//        if(get_pattern_mode_index_by_string(sdi->mode , stropt) != PATTERN_INVALID)
//        {
